...
const AffectationForm: React.FC<Props> = ({ employeId, onAffectationComplete }) => {
  ...
  // Chargement initial des tournées
  useEffect(() => {
    const charger = async () => {
      try {
  ...
  // Mise à jour des points d’arrêt lorsque la tournée sélectionnée change
  useEffect(() => {
    if (!selectedTourneeId || selectedTourneeId === 'undefined') return

    const tournee = tournees.find((t) => t.id && t.id === selectedTourneeId)
    if (tournee) {
      setTourneeType(tournee.type)

      if (tournee.type === 'fixe' && tournee.id) {
        console.log('🔁 Chargement des points d’arrêt pour la tournée :', tournee.id)
        ...
        console.warn("⚠️ tournee.id est invalide, requête non envoyée.")
      }
    }
  }, [selectedTourneeId, tournees])

  const handleAffectation = async () => {
    if (!selectedTourneeId || selectedTourneeId === 'undefined') {
      return toast.error('Veuillez choisir une tournée')
    }
  ...
  return (
    <div className="space-y-4 bg-white border rounded shadow p-4 mt-4">
      ...
      {/* Champ ordre embarquement pour tournée flexible */}
      ...
      {/* Sélecteur de point d’arrêt pour tournée fixe */}
      {tourneeType === 'fixe' && (
        <div>
      ...
      <Button type="button" onClick={handleAffectation}>
        Affecter
      </Button>
    </div>
  )
