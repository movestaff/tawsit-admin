...
export async function updatePrestataire(id: string, data: any) {
  ...
  if (!response.ok) throw new Error('Erreur lors de la mise à jour du prestataire');
...
export async function deletePrestataire(id: string) {
  ...
  if (!response.ok) throw new Error('Erreur lors de la suppression du prestataire');
  return await response.json();
}